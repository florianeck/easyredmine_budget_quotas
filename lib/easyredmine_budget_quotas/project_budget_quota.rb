module EasyredmineBudgetQuotas
  module ProjectBudgetQuota
    ...
    def query_spent_entries_on(type:)
      current_entry = self.send("current_#{type}_entry")
      ...
      if current_entry
        ...
        query << "WHERE `custom_fields`.`internal_name` = 'ebq_budget_quota_source' AND `custom_values`.`value` = 'quota' AND `time_entries`.`spent_on` BETWEEN '#{current_entry.valid_from}' AND '#{current_entry.valid_to}' AND `time_entries`.`project_id` = #{self.id}"
        query << "AND `easy_money_time_entries_expenses`.`rate_type_id` = #{self.external_rate_id}"
    ...
    def external_rate_id
      EasyMoneyRateType.find_by(name: 'external').id
    end
    ...
    def current_budget_entry
      get_current_budget_quota_entries(type: :budget)
    end

    def current_quota_entry
      get_current_budget_quota_entries(type: :quota)
    end

    def get_current_budget_quota_entries(type:)
